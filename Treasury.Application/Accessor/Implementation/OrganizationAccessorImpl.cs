...
public class OrganizationAccessorImpl : IOrganizationAccessor
{
    ...
    {
        var skip = GeneralHelperFunctions.GetPage(generalPagedRequest);
    ...
    private static IQueryable<Organization> ApplyFilters(GeneralPagedRequest request,
        DbSet<Organization> orderedQueryable)
    {
        ...
        if (request.Acronym.Length > 0)
        {
            ...
            predicate = request.Acronym.Aggregate(predicate,
                (current, acronym) => current.Or(p => p.Acronym1.Contains(acronym)));
        ...
        if (request.Classification.Length > 0)
        {
            ...
            predicate = request.Classification.Aggregate(predicate,
                (current, classification) => current.Or(p => p.Classification.Contains(classification)));
        ...
        if (request.Type.Length > 0)
        {
            ...
            predicate = request.Type.Aggregate(predicate,
                (current, type) => current.Or(p => p.TypeOfClub.Contains(type)));

            filtered = filtered.Where(predicate);
        }

        if (!request.IncludeInactive) filtered = filtered.Where(query => !query.Inactive);
